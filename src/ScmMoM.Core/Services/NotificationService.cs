...
public class NotificationService
{
    ...
    public List<NotificationEvent> DetectFailedCiRuns(IReadOnlyList<CiRunInfo> current)
    {
        var currentFailed = current
            .Where(a => a.Conclusion is "failure")
            .ToList();

        var currentKeys = currentFailed.Select(GetCiRunKey).ToHashSet();
        var newFailures = currentFailed
            .Where(a => !_previousFailedActionKeys.Contains(GetCiRunKey(a)))
    ...
    private static string GetCiRunKey(CiRunInfo a) => $"{a.RepoName}/{a.WorkflowName}/{a.RunNumber}";
}
